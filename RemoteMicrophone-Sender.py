"""
Remote microphone sender for the Local PC.

Captures audio and streams it to the Remote PC.
...
class SenderApp:
    def __init__(self, root):
        ...
        ttk.Label(root, text="Remote PC IP address:").grid(
    ...
    def _set_connection_controls_enabled(self, enabled):
        self.mic_combo.config(state="readonly" if enabled else "disabled")
        self.ip_entry.config(state="normal" if enabled else "disabled")
        self.port_entry.config(state="normal" if enabled else "disabled")

    def start(self):
        ip = self.ip_var.get().strip()
        if not ip:
            messagebox.showerror("Error", "Enter the remote PC's IP address.")
            return
        ...
        # Downmix stereo input to the mono packet format.
        ...
        def audio_callback(indata, frames, time_info, status):
            ...
            if input_channels == 1 and gain == 1.0:
    ...
    def _poll_status(self):
        if not self.running or not self.stream or not self.destination:
            return
        ...
        ip, port = self.destination
        self.status_var.set(
            f"Streaming to {ip}:{port} "
            f"(drops {self.queue_drop_count}, input flags {self.input_status_count}"
            f"{metrics})"
        )
