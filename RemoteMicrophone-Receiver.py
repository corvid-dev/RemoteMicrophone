"""
Remote microphone receiver for the Remote PC.

Receives audio and plays it through the selected output device.
...
class PacketRingBuffer:
    ...
    def insert(self, sequence, packet, expected_sequence):
        ...
        # Treat wrapped values behind the reader as stale.
...
class ReceiverApp:
    def __init__(self, root):
        ...
        self.trim_count = 0
        self.output_status_count = 0

        pad = {"padx": 10, "pady": 6}

        ttk.Label(root, text="Remote PC IP (enter on Local PC):").grid(
    ...
    def start(self):
        ...
        # Duplicate mono packets across stereo output channels.
        ...
        def receive_loop():
            while not self.stop_event.is_set():
                ...
                packet_data = (audio, rms <= SILENCE_RMS_THRESHOLD)
                ...
                with self.buffer_lock:
                    ...
                    result = self.packet_buffer.insert(
                        sequence, packet_data, self.expected_sequence
                    )

                    # Reset to the newest packet after a large sequence jump.
                    if result == "too_far":
                        self.packet_buffer.clear()
                        self.expected_sequence = sequence
                        self.playback_started = False
        ...
        def audio_callback(outdata, frames, time_info, status):
            ...
            with self.buffer_lock:
                ...
                buffered = self.packet_buffer.contiguous_count(self.expected_sequence)
                now = time.monotonic()
                if (
                ...
                packet_data = self.packet_buffer.pop(self.expected_sequence)
                if packet_data is None:
                    self.underrun_count += 1
                    # Rebuild the target buffer after a missing packet.
                    self.playback_started = False
                    return
    ...
    def _poll_status(self):
        ...
        if sender_active:
            ...
            try:
                ...
                cpu_percent = float(self.stream.cpu_load) * 100.0
                metrics = f", latency {latency_ms:.1f} ms, CPU {cpu_percent:.1f}%"
            except Exception:
                metrics = ""
            ...
            self.status_var.set(
                ...
                f"trims {self.trim_count}, output flags {self.output_status_count}"
                f"{metrics})"
            )
        else:
            self.last_sender_ip = None
